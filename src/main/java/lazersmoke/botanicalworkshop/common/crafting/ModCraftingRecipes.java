...
public class ModCraftingRecipes{
	...
	public static void init(){
		...
		if(LibConfigs.CRAFT_ELVEN_POOL){
			addOreDictRecipe(new ItemStack(ModBlocks.elvenPool), "Q Q", "QQQ",
					'Q', new ItemStack(
							vazkii.botania.common.item.ModItems.quartz, 1, 5));
		...
		if(LibConfigs.CRAFT_SIMPLE_CATALYST){
			addShapelessOreDictRecipe(new ItemStack(ModItems.simpleCatalyst),
					new ItemStack(Items.glowstone_dust), new ItemStack(
							Items.redstone), new ItemStack(Items.gunpowder),
					new ItemStack(
		...
		if(LibConfigs.CRAFT_EMPTY_CATALYST){
			addShapelessOreDictRecipe(new ItemStack(ModItems.emptyCatalyst),
					new ItemStack(ModItems.simpleCatalyst), new ItemStack(
							Items.bowl));
			recipeEmptyCatalyst = BotaniaAPI.getLatestAddedRecipe();
		}

		if(LibConfigs.CRAFT_WEAK_GATEWAY_CORE){
			addOreDictRecipe(new ItemStack(ModBlocks.weakGatewayCore), "DGD",
					"LEL", "DGD", 'D', new ItemStack(Items.diamond), 'G',
					new ItemStack(Items.gold_ingot), 'L', new ItemStack(
							Items.dye, 1, 4), // Lapis
					'E', new ItemStack(
