...
public class ModGatewayTransmutationRecipes{
	...
	public static List<RecipeGatewayTransmutation> vanillaRecipes = new ArrayList<RecipeGatewayTransmutation>();
	...
	public static void initVanilla(){
		if(LibConfigs.GATEWAY_CRAFT_VANILLA_RECIPES){
			vanillaRecipes.clear();
			for(final IRecipe currRecipe : (List<IRecipe>) CraftingManager.getInstance().getRecipeList()){// getRecipeList always return list of IRecipe
				if(currRecipe instanceof ShapelessRecipes)
					if(((ShapelessRecipes) currRecipe).getRecipeOutput() != null){
						final List<ItemStack> localInputItems = new ArrayList<ItemStack>(((ShapelessRecipes) currRecipe).recipeItems);// recipeItems is always an List<ItemStack>
						localInputItems.removeAll(Collections.singleton(null));// Remove nulls
						localInputItems.add(new ItemStack(ModItems.botanicalResource, 1, 4));// Add a vanilla reagent
						vanillaRecipes.add(BotanicalWorkshopAPI.registerGatewayRecipe(((ShapelessRecipes) currRecipe).getRecipeOutput(), new ItemStack(ModItems.simpleCatalyst), localInputItems));
					}
				if(currRecipe instanceof ShapedRecipes)
					if(((ShapedRecipes) currRecipe).getRecipeOutput() != null){
						final List<ItemStack> localInputItems = new ArrayList<ItemStack>(Arrays.asList(((ShapedRecipes) currRecipe).recipeItems.clone()));
						localInputItems.removeAll(Collections.singleton(null));// Remove nulls
						localInputItems.add(new ItemStack(ModItems.botanicalResource, 1, 4));// Add a vanilla reagent
						vanillaRecipes.add(BotanicalWorkshopAPI.registerGatewayRecipe(((ShapedRecipes) currRecipe).getRecipeOutput(), new ItemStack(ModItems.simpleCatalyst), localInputItems));
					}
			}
		}
	}
	...
	public static void init(){
		...
		if(LibConfigs.GATEWAY_CRAFT_LIGHTNING_TRANSFORMER)
			lightningTransformerRecipe = BotanicalWorkshopAPI.registerGatewayRecipe(
				...
				new ItemStack(ModItems.botanicalResource, 2, 3)
				);
