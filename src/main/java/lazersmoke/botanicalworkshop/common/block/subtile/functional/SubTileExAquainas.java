...
public class SubTileExAquainas extends TonalFunctionalFlower{
	...
	public void onUpdate(){
		if(firstTick){
			pitches = new float[][] {{0.9F, 1.2F, 1.4F, 1.32F, 1.2F, 0.95F, 0.9F, 0.8F, 0.7F, 0.67F, 0.9F, 0.9F, 1.2F, 1.4F,
					1.32F, 1.2F, 0.95F, 0.9F, 0.8F, 0.7F, 1.1F, 0.9F, 0.9F, 0.8F, 0.7F, 1.05F, 0.7F, 0.8F, 0.9F, 0.95F, 0.9F,
					0.8F, 0.7F, 0.67F, 0.6F, 0.67F, 0.7F, 0.95F, 0.9F, 1.1F, 1.32F, 1.9F, 1.8F}};
			times = new int[][] {{4, 6, 1, 1, 2, 4, 2, 6, 1, 1, 4, 4, 6, 1, 1, 2, 4, 2, 6, 1, 1, 4, 4, 6, 1, 1, 4, 4, 6, 1, 1, 4,
		...
		if(((activeTile instanceof IPetalApothecary && ((IPetalApothecary) activeTile).hasWater() == false) || (activeTile instanceof IFluidHandler && (((IFluidHandler) activeTile)).canFill(ForgeDirection.DOWN, FluidRegistry.WATER)) || hasBucket || redstoneSignal > 0) && mana >= COST && !supertile.getWorldObj().isRemote && ticksExisted % 10 == 1){
			final List<int[]> offsets = Arrays.asList(OFFSETS);
			Collections.shuffle(offsets);

			for(final int[] offsetArray : offsets){
				final int[] positions = {supertile.xCoord + offsetArray[0], supertile.zCoord + offsetArray[1]};
				final Block search = Blocks.water;
				if(supertile.getWorldObj().getBlock(positions[0], supertile.yCoord, positions[1]) == search && supertile.getWorldObj().getBlockMetadata(positions[0], supertile.yCoord, positions[1]) == 0){
					int waterAround = 0;
					for(final ForgeDirection dir : LibMisc.CARDINAL_DIRECTIONS)
