...
public class TileLightningTransformer extends TileModLightning implements IBotanicalLightningBlock{
	...
	public int getLightningPushRange(){
		return ((worldObj.getTileEntity(xCoord, yCoord + 1, zCoord) instanceof IBotanicalLightningBlock) &&
				(((IBotanicalLightningBlock) worldObj.getTileEntity(xCoord, yCoord + 1, zCoord)).getConductivity() > 0)
				)? 1 : 0;//If the block above me is at least 1 conductive, then push to it
	}
}
