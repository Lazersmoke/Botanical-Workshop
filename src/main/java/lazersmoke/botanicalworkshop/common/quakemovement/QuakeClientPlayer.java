...
public class QuakeClientPlayer extends ClientPlayerBase {
	...
	public void moveEntityWithHeading(float sidemove, float forwardmove) {
		if (ItemNBTHelper.getBoolean(this.player.getEquipmentInSlot(1), "shiftedUpgradeHopsUpgrade", false)
				&& ItemShiftedArmor.getCore(this.player.getEquipmentInSlot(1), this.player.worldObj) != null && ItemShiftedArmor.getCore(this.player.getEquipmentInSlot(1), this.player.worldObj).isOpen()) {
	...
	public float getSurfaceFriction() {
		...
		if (this.player.onGround) {
			Block ground = this.player.worldObj.getBlock(MathHelper.floor_double(this.player.posX), MathHelper.floor_double(this.player.boundingBox.minY) - 1,
	...
	private void spawnBunnyhopParticles(int numParticles) {
		...
		if (ground != null && ground.getMaterial() != Material.air) {
			for (int iParticle = 0; iParticle < numParticles; iParticle++) {
				this.player.worldObj.spawnParticle("blockcrack_" + Block.getIdFromBlock(ground) + "_" + this.player.worldObj.getBlockMetadata(j, i, k), this.player.posX
						+ (this.playerAPI.getRandField().nextFloat() - 0.5D) * this.player.width, this.player.boundingBox.minY + 0.1D, this.player.posZ
						+ (this.playerAPI.getRandField().nextFloat() - 0.5D) * this.player.width, -this.player.motionX * 4.0D, 1.5D, -this.player.motionZ * 4.0D);
			}
		}
	}
	...
	private void minecraft_ApplyGravity() {
		if (this.player.worldObj.isRemote
				&& (!this.player.worldObj.blockExists((int) this.player.posX, 0, (int) this.player.posZ) || !this.player.worldObj.getChunkFromBlockCoords((int) this.player.posX,
	...
	public void minecraft_moveEntityWithHeading(float sidemove, float forwardmove) {
		...
		if ((this.player.isInWater() && !this.player.capabilities.isFlying) || (this.player.handleLavaMovement() && !this.player.capabilities.isFlying)) {
			super.moveEntityWithHeading(sidemove, forwardmove);
		} else {
			// get friction
	...
	public void quake_moveEntityWithHeading(float sidemove, float forwardmove) {
		...
		if ((this.player.handleLavaMovement() && !this.player.capabilities.isFlying)) {
			...
			// ground movement
			if (onGroundForReal) {
				// apply friction before acceleration so we can accelerate back
				// up to maxspeed afterwards
				// quake_Friction(); // buggy because material-based friction
				// uses a totally different format
				minecraft_ApplyFriction(momentumRetention);
			...
			// air movement
				...
				if (QuakeConfig.SHARKING_ENABLED && QuakeConfig.SHARKING_SURFACE_TENSION > 0.0D && this.playerAPI.getIsJumpingField() && this.player.motionY < 0.0F) {
			...
			// HL2 code applies half gravity before acceleration and half after
			// acceleration, but this seems to work fine
			minecraft_ApplyGravity();
		}
	...
	private boolean quake_DoTrimp() {
		if (QuakeConfig.TRIMPING_ENABLED && this.player.isSneaking()) {
			...
			if (curspeed > movespeed) {
				...
				this.player.motionY += speedbonus * curspeed * QuakeConfig.TRIMP_MULTIPLIER;
	...
	private void quake_ApplyWaterFriction(double friction) {
		...
		/*
		 * float speed = (float)(this.player.getSpeed()); float newspeed = 0.0F;
		 * if (speed != 0.0F) { newspeed = speed - 0.05F * speed * friction; //*
		 * player->m_surfaceFriction; float mult = newspeed/speed;
		 * this.player.motionX *= mult; this.player.motionY *= mult;
		 * this.player.motionZ *= mult; } return newspeed;
		 */

		/*
		 * // slow in water this.player.motionX *= 0.800000011920929D;
		 * this.player.motionY *= 0.800000011920929D; this.player.motionZ *=
		 * 0.800000011920929D;
		 */
	}
	...
	private void quake_WaterMove(float sidemove, float forwardmove) {
		...
		if (!isSharking || curspeed < 0.078F) {
			...
			if (curspeed > 0.098)
				quake_AirAccelerate(wishspeed, wishdir[0], wishdir[1], QuakeConfig.ACCELERATE);
			else
				quake_Accelerate(.0980F, wishdir[0], wishdir[1], QuakeConfig.ACCELERATE);
		...
		if (this.player.isCollidedHorizontally
				&& this.player.isOffsetPositionInLiquid(this.player.motionX, this.player.motionY + 0.6000000238418579D - this.player.posY + lastPosY, this.player.motionZ)) {
			this.player.motionY = 0.30000001192092896D;
		}
	...
	private void quake_ApplySoftCap(float movespeed) {
		...
		// apply soft cap first; if soft -> hard is not done, then you can
		// continually trigger only the hard cap and stay at the hard cap
		if (speed > softCap) {
