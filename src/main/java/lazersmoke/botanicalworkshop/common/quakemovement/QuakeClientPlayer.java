...
public class QuakeClientPlayer extends ClientPlayerBase{
	...
	public void moveEntityWithHeading(float sidemove, float forwardmove){
		if(ItemNBTHelper.getBoolean(this.player.getEquipmentInSlot(1), "shiftedUpgradeHopsUpgrade", false) && ItemShiftedArmor.getCore(this.player.getEquipmentInSlot(1), this.player.worldObj) != null && ItemShiftedArmor.getCore(this.player.getEquipmentInSlot(1), this.player.worldObj).isOpen()){
	...
	private void quake_ApplyWaterFriction(double friction){
		...
		/*
		 * float speed = (float)(this.player.getSpeed()); float newspeed = 0.0F; if (speed != 0.0F) { newspeed = speed - 0.05F * speed * friction; //* player->m_surfaceFriction; float mult = newspeed/speed; this.player.motionX *= mult; this.player.motionY *= mult; this.player.motionZ *= mult; } return newspeed;
		 */
